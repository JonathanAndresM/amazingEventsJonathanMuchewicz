...
card(upcomingEvents, cardContainer)
...
/*function createCard(cardContainer, card) {
  ...
  divCard.innerHTML = `<img src="${card.image}" class="card-img-top rounded-top img-card" alt = "Metallica in concert" >
    ...
    <div class="card-body d-flex justify-content-between align-items-center h-25">
...
function filterEvents() {
  ...
  let searchFilter = upcomingEvents.filter(event => event.name.toLowerCase().includes(search.value.toLowerCase()) || event.description.toLowerCase().includes(search.value.toLowerCase()))
